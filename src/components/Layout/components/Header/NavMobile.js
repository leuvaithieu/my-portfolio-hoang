const NavMobile = (props) => {
  const { isShow=false, onHide, navLinks, selectedLink, onSelectLink } = props;
  return <div className={`fixed top-0 w-full  h-screen z-[1000] -left-full ${
    isShow ? "translate-x-full" : ""
  } transition-transform duration-500`}>
      <div className="relative w-full h-full">
        <div className={`absolute w-full h-full bg-slate-900  z-[0] opacity-0 ${
            isShow ? "opacity-40" : ""
          } duration-1000`}
          onClick={() => {
            console.log("click");
          ...
          <div className="absolute bg-slate-100 dark:bg-slate-900 h-full w-[80%] flex  flex-col justify-between z-[1]">
              ...
              <div className="sticky inset-x-0 bottom-0 border-t border-gray-100">
                <a className="flex items-center gap-2 p-4 ">
                  <div>
                    <p className="text-xs">
                      <strong className="block font-medium">HoangLC</strong>

                      <span> leuvaithieu95@gmail.com </span>
                    </p>
                  </div>
                </a>
              </div>
